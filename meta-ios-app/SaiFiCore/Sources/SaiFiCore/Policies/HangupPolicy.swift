...
//
// Ported from Android `HangupPolicy.kt`.

/// What `HangupPolicy.decide` concluded about an `endCall`.
public enum HangupAction: Equatable, Sendable {
  ...
  case holdAndAsk(why: String, nudge: String)
...
public enum HangupPolicy {

  public static func decide(
    spokeThisTurn: Bool,
    lastUserSpeechAt: Int64,
    lastSaiSpeechAt: Int64,
    lastSaiText: String,
    muted: Bool,
    guardUsed: Bool
  ) -> HangupAction {
    ...
    let saidGoodbye =
      spokeThisTurn
      || (lastSaiSpeechAt > lastUserSpeechAt && !lastSaiText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
    let userAsked = lastUserSpeechAt != 0

    if (saidGoodbye && userAsked) || guardUsed {
      return muted ? .endNow : .endAfterGoodbye
    }

    let why =
      !userAsked
      ? "the user hasn't said anything this call"
      : "Sai hasn't spoken since the user's last turn — no goodbye"
    if muted { return .holdSilently(why: why) }
    return .holdAndAsk(why: why, nudge: unconfirmedNudge)
  }
  ...
  public static func shouldCancel(openedAt: Int64, now: Int64, stragglerGuardMs: Int64) -> Bool {
  ...
  public static let unconfirmedNudge =
  ...
  public static let cancelledNudge =
