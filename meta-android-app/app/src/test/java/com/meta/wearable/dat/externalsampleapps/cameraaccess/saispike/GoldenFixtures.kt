...
//
// That split is the point: generation used to be an ordinary test that wrote its own expected
// output, so every CI run silently rewrote the fixtures and drift became undetectable. Generation
// is an explicit, env-gated command here; the asserting tests only read.
//
// The wording is load-bearing — nearly all of it found by hearing it fail on a real call — so a
// change to it should be a diff someone reviews rather than something a user notices on the glasses.
//
  ...
  val scenarios =
      listOf(
          ...
          Scenario(
              ...
              readAt = T0 + 2_000),
          // A finished result must outlive the buffer it was reported in. `getSaiStatus` is where the
          // ask-first nudge sends the model when it is holding a result and the user finally asks, so
          // a result that has scrolled away is an assistant that says the task stopped — which is
          // what a user heard on 2026-08-20. maxLines=3 makes the overflow happen in four steps
          // instead of thirteen; a real second task pushes the line out just as surely.
          Scenario(
              name = "a finished result outlives the buffer the next task fills",
              maxLines = 3,
              timeline =
                  listOf(
                      Step(T0, jobj("type" to jstr("status"), "status" to jstr("processing"))),
                      Step(
                          T0 + 100,
                          jobj(
                              "type" to jstr("complete"),
                              "summary" to jstr("3 unread, all newsletters"))),
                      // A second task, which is what pushes the completion out of the scrollback.
                      Step(T0 + 200, jobj("type" to jstr("status"), "status" to jstr("processing"))),
                      Step(
                          T0 + 300,
                          jobj("type" to jstr("progress"), "text" to jstr("opening the browser"))),
                      Step(
                          T0 + 400,
                          jobj("type" to jstr("progress"), "text" to jstr("reading the page"))),
                      Step(
                          T0 + 500,
                          jobj("type" to jstr("progress"), "text" to jstr("filling the form"))),
                  ),
              readAt = T0 + 1_000),
