...
import com.meta.wearable.dat.externalsampleapps.cameraaccess.saispike.CaptureNotes
...
val TRANSCRIPTS: List<Transcript> =
    listOf(
        ...
        // Camera clipboard: a capture with no request attached to it. It should confirm it HAS the
        ...
        // "nothing happened" is false — but it was never told where the user is, so naming a city
        ...
        Transcript(
            ...
            // It already has the user's answer to relay; there is nothing to re-forward and nothing
            // to capture. The city in the last turn is the USER's, which it may repeat.
        ...
        // Overheard speech: none of these turns is addressed to it, so there is nothing to act on
        // and nothing worth saying. The failure this guards is what it does with such a turn — the
        // prompt used to advertise a `noop` tool that wasn't declared, and with nothing to call it
        ...
        // lunch?" reads as asking Sai to find lunch, which the prompt rightly forwards as a "near
        ...
        // Device 2026-08-20: the wearer was booking a flight, then talking to someone else ABOUT
        // Sai ("do you want it to actually show the results?", "I think it's hearing itself",
        // "it doesn't notice y'all talking"). The overheard-speech rule used to require the talk be
        // "unrelated to any task", so related side-chat was treated as addressed, and Sai kept
        // re-asking which weekend after every barge-in. Dana is named for the same reason as the
        // lunch transcript: text mode has no ambient channel.
        Transcript(
            name = "side conversation about the current task — still not for Sai",
            targets = listOf("only-addressed-speech", "no-verbalized-tool", "first-person"),
            turns =
                listOf(
                    user("help me book a flight to boston"),
                    agent(status("processing")),
                    user("Dana, do you want it to actually show the results? Because it'll take quite a while."),
                    user("Oh, okay. Yeah, that's how long it takes."),
                    user("I think it's hearing itself."),
                    user("Okay, but it's quite smart in that it doesn't notice y'all talking."),
                ),
            expectTools =
                ToolExpectation(
                    atMost = mapOf("forwardToAgent" to 1),
                    excludes = listOf("relayToAgent", "interrupt", "askAndWait", "endCall"))),
        ...
        // The SHORTER, more elliptical phrasing — and the one that actually failed on device: it
        ...
        Transcript(
            ...
            expectTools =
                ToolExpectation(
                    includes = listOf("captureImage", "forwardToAgent"),
                    flags = mapOf("forwardToAgent" to mapOf("attachLatestImage" to true)))),
        // Device 2026-08-20, two flower-ID runs. RUN 1 spoke the silent task text ("Paragraph
        // writing about the flower species") without forwarding. RUN 2 asked "is there anything
        // else you wanted me to do with it?" — the old THEN ASK ONE QUESTION, asked of the user.
        // "Identify this flower" already IS the beyond-the-picture work: capture AND forward,
        // never interrogate, never speak the task.
        Transcript(
            name = "identify this flower — capture and forward, never speak the task or ask what's next",
            targets = listOf("no-tool-narration", "no-filler", "capture-not-just-talk"),
            turns = listOf(user("Help me identify this flower.")),
            toolResults =
                mapOf(
                    "captureImage" to
                        JSONObject()
                            .put("result", "capture-started")
                            .put("note", CaptureNotes.STARTED)),
            expectTools =
                ToolExpectation(
                    includes = listOf("captureImage", "forwardToAgent"),
                    excludes = listOf("askAndWait"),
                    flags = mapOf("forwardToAgent" to mapOf("attachLatestImage" to true)))),
        // Device regression, 2026-07-30: straight after the muted nudge it produced
        ...
        // produce no speech, it wrote a token into the turn instead of leaving it empty. Muted is
        // the only state where that happens, so it needs a transcript that actually enters it.
        //
        // The overheard remark in the middle is what used to draw the placeholder: a turn it must
        ...
        Transcript(
            ...
            // It may keep working while muted, but nothing here asks for a new task or a hang-up.
        ...
        // weather task's tool step failed, nothing had come back, and asked about it Sai produced
        ...
        //
        // It was told nothing at the time; that hole is closed (a failed step now reaches it as
        // context). This transcript is the other half — given the fact, it must say it hasn't got a
        ...
        Transcript(
            ...
            // Pressed twice, it may re-check or re-run — but it must not answer from nothing.
            expectTools = ToolExpectation(excludes = listOf("endCall"))),
        ...
        // goldens. What CANNOT be checked there is the model's half: whether it describes a task
        ...
        // Failure mode 2 from the plan: told that forwardToAgent now queues, it may start phrasing
        ...
        Transcript(
            ...
            // Two separate requests are two forwards. A relay would be it routing around the queue.
            expectTools =
                ToolExpectation(
                    includes = listOf("forwardToAgent"),
                    excludes = listOf("relayToAgent"),
                    atMost = mapOf("forwardToAgent" to 2))),
        // The E1 log's exact question, with the answer now available to it. `session-state` is what
        ...
        // Device 2026-07-31: with the booking parked on "which restaurant?", it said "Got it, I'm
        ...
        // reporting half — the wait is on the USER, for a question it asked itself, and nothing is
        // in progress.
        Transcript(
            name = "parked on the user — the wait is Sai's to end, not a third party to hear back from",
            targets =
                listOf(
            ...
            // until it resolves. It answers from what it already has and puts the question back.
            expectTools =
                ToolExpectation(
