...
//
// THIS COMPLEMENTS `LoopEvalTest`; IT DOES NOT REPLACE IT. This one drives 33 transcripts with no FSM
...
// The asymmetry is what makes them worth keeping apart — this one can grade whether she SAYS the
      ...
      while (true) {
        ...
        chat.addToolResponses(
            calls.map { c ->
              ...
              val response =
                  t.toolResults[name]
                      ?: if (name == "getSaiStatus") {
                        JSONObject().put("status", activity.statusText())
                      } else {
      ...
      when (turn) {
        ...
        is Turn.Agent -> {
          ...
          if (muted && turn.event.optString("type") == "complete") {
            // Held, not dropped: the device replays it once she is audible, using the ask-first
            // wording — offer the result when they are free, rather than announcing into a room she
