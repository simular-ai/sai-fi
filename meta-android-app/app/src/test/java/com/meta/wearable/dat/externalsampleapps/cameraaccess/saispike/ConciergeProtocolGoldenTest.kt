...
class ConciergeProtocolGoldenTest {
  ...
  fun askFirstNudge_demandsSilenceThenOneShortOffer() {
    ...
    // …and the silence has to END. Live on 2026-08-20 Sai held a finished result correctly, then
    // told the user the task had STOPPED when they asked how it was going, and offered to start
    // again — the result never reached them. Told only to wait, the model has nothing saying the
    // waiting is over, so all three clauses that end it are asserted here.
    assertTrue(e.contains("DELAY, NOT A DISCARD"))
    assertTrue(e.contains("that IS the gap you were waiting for"))
    assertTrue(e.contains("never say it stopped"))
    // The same escape hatch the ordinary completion nudge has: a summary can announce a result
    // without carrying one, and the way out is to look, not to fill the gap.
    assertTrue(e.contains("does not actually CONTAIN the answer"))
    assertTrue(e.contains("getSaiStatus"))
  }

  @Test
  fun askFirstNudge_withNoResultHoldsNothingBack() {
    // An absent summary used to be handed over as the literal word "done", fenced as the RESULT —
    // so a turn that reported nothing arrived looking like a result that said nothing, under orders
    // to offer it later. There is nothing to offer, and this variant says so.
    val e = expected("complete-ask-first.json", "ask-first without summary")
    assertTrue(e.contains("WITHOUT reporting any result"))
    assertTrue(e.contains("do NOT invent what it might have found"))
    assertTrue("nothing is being held, so there is no result to fence", !e.contains("\"\"\""))
  }
  ...
  fun systemNudgeConstants_sayWhatTheyMust() {
    ...
    // Muting must stop it SPEAKING without stopping it listening or working — and must not be
